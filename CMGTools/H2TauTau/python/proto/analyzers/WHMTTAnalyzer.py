...
#        print 'Init for the WHMTTAnalyzer'
        super(WHMTTAnalyzer,self).__init__(cfg_ana, cfg_comp, looperName)


    # beginLoop
    def beginLoop(self):
#       print 'Init for the beginLoop'
        super(WHMTTAnalyzer, self).beginLoop()
        self.counters.addCounter('MTT')
        count = self.counters.counter('MTT')
    ...
    def declareHandles(self):
        super(WHMTTAnalyzer, self).declareHandles()
    ...
    def buildLooseLeptons(self, cmgLeptons, event):
        ...
        for index, lep in enumerate(cmgLeptons):
            ...
            if pyl.pt() > 10. and abs(pyl.eta()) < 2.4 and \
                   ...
                   abs(pyl.dB3D()) < 0.2 and abs(pyl.dxy()) < 0.045 and \
...
#	import pdb; pdb.set_trace()
        ...
        self.counters.counter('MTT').inc('all events')
...
#        self.counters.counter('MTT').inc('1mu + 1e')
#        
#
#        event.muon = ''
#        event.electron = ''
...
#                if im.charge()*it.charge()==1:
#                    flag_sscharge = True

        self.counters.counter('MTT').inc('step1')
...
#        for it in event.taucand:
#            if it.charge()==1:
#                flag_charge_plus = True
#            elif it.charge()==-1:
#                flag_charge_minus = True
#            else:
#                print 'Imvalid tau charge', it.charge()
#                
#

        self.counters.counter('MTT').inc('step2')

#        if not len(event.vetoelectroncand)==0:
#            return False
        
#        if not (len(event.taucand)>=1 and len(event.muoncand)>=1 and len(event.electroncand)>=1):
        if not (len(event.muoncand)>=1):
            return False

        self.counters.counter('MTT').inc('step3')
...
#        self.counters.counter('MTT').inc('1 e/mu/tau')
#        event.tau = lepton3[0]
